...
import org.apache.hadoop.mapreduce.Mapper.Context;
	 ...
	 * WordMapperIdf.java
	 */
	public class WordMapperIdf extends Mapper<LongWritable, Text, Text, Text>{
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException{
			...
			if(str.length > 2){
				...
				String TermF = str[2];												//Separate Term Frequency
				
				Text OutKey = new Text (Words);										//Pass Author Last Name And Word as Key	
				Text OutValue = new Text (Author+"\t"+TermF);						//Pass Word And Count As Value
